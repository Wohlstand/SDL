# Mac OS X Tiger (10.14) with PowerPC and x86 processors.

These instructions are for people using LEGACY Apple's Mac OS X Tiger.

If you looking for the modern macOS support (the minimum version
10.7 is required), please follow the "README-macos.md" file instead
of this.
...
Unlike the modern Mac OS X (now called macOS) build, this version will
introduce some limitations compared to the modern Mac OS X version,
for example, lack of joysticks support and X11 video instead of the
Cocoa. So, you should install the X11 environment to your system.

Before to start, you will need to install the Xcode 2.5+ and set up the
Tigerbrew environment: the fork of the Homebrew for modern macOS
targeted to legacy Mac OS X Tiger environment:
...
To build SDL using the command line, use the CMake, although it continues
...
Please note that building Mac Tiger SDL requires at least Xcode 2.5 and
the 10.4 SDK.
...
Your SDL app is essentially an X11 application like on other UNIX-like
operating systems. You are required to run the X11.app and properly set
the DISPLAY environment variable (for example, `export DISPLAY=:0`) to get
